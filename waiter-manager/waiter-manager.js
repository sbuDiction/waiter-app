...
module.exports = function Waiter_manage_system(pool) {
    ...
    const add_shift = async (name, day) => {
        ...
        let get_by_name = await services.get_by_name(name)
        ...
        for (var x = 0; x < days_array.length; x++) {
            var get_by_day = await services.get_by_day(days_array[x])
    ...
    const display_counter = async days_id => {
        let days = await services.get_by_day(days_id)
    ...
    const display_waiters = async () => {
        let waiters = await services.get_all_names()
        return waiters;
    };

    const display_days = async () => {
        let days = await services.get_all_days()
        return days;
    };
    ...
    const display_workers = async name => {
        let days_object_list = [{
            days_in_a_week: "Monday",
            days_count: 0,
            status: "#ffff00",
            waiters: []
        },
        {
            days_in_a_week: "Tuesday",
            days_count: 0,
            status: "#ffff00",
            waiters: []
        },
        {
            days_in_a_week: "Wednesday",
            days_count: 0,
            status: "#ffff00",
            waiters: []
        },
        {
            days_in_a_week: "Thursday",
            days_count: 0,
            status: "#ffff00",
            waiters: []
        },
        {
            days_in_a_week: "Friday",
            days_count: 0,
            status: "#ffff00",
            waiters: []
        },
        {
            days_in_a_week: "Saturday",
            days_count: 0,
            status: "#ffff00",
            waiters: []
        },
        {
            days_in_a_week: "Sunday",
            days_count: 0,
            status: "#ffff00",
            waiters: []
        }
        ...
        let get_by_id = await services.select_name(name)
        ...
        for (let x = 0; x < show_all.rows.length; x++) {
            for (var i = 0; i < days_object_list.length; i++) {
                if (show_all.rows[x].days_in_a_week === days_object_list[i].days_in_a_week) {
                    ...
                    days_object_list[i].days_count = await display_counter(
                        show_all.rows[x].days_in_a_week
                    );
                    if (days_object_list[i].days_count == 3) {
                        days_object_list[i].status = "#008000";
                    }
    ...
    const return_checked = async name => {
        let days_object_list = [{
            days_in_a_week: "Monday",
            element_id: "#defaultInline1",
            state: "null"
        },
        {
            days_in_a_week: "Tuesday",
            element_id: "#defaultInline2",
            state: "null"
        },
        {
            days_in_a_week: "Wednesday",
            element_id: "#defaultInline3",
            state: "null"
        },
        {
            days_in_a_week: "Thursday",
            element_id: "#defaultInline4",
            state: "null"
        },
        {
            days_in_a_week: "Friday",
            element_id: "#defaultInline5",
            state: "null"
        },
        {
            days_in_a_week: "Saturday",
            element_id: "#defaultInline6",
            state: "null"
        },
        {
            days_in_a_week: "Sunday",
            element_id: "#defaultInline7",
            state: "null"
        }
        ];

        let which_box = await services.select_name(name)
